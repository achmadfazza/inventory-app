            ...
            <td>
              ...
              <x-modal id="{{ $user->id }}" title="Ubah Data">
                <form action="{{ route('backoffice.user.update', $user->id) }}" method="POST">
                  @csrf
                  @method('PUT')
                  <x-input title="Nama User" name="name" type="text" placeholder="Masukan Nama user" value="{{ $user->name }}" />
                  <x-select-group title="Role">
                    @foreach ($roles as $role)
                    <label class="form-selectgroup-item">
                      <input type="checkbox" name="roles[]" value="{{ $role->id }}" class="form-selectgroup-input" @checked($user->roles()->find($role->id))>
