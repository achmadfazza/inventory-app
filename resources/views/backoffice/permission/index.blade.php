          ...
          <tr>
            ...
            <td>
              ...
              <x-modal id="{{ $permission->id }}" title="Ubah Data">
                <form action="{{ route('backoffice.permission.update', $permission->id) }}" method="POST">
                  @csrf
                  @method('PUT')
                  <x-input title="Nama Perizinan" name="name" type="text" placeholder="Masukan Nama Perizinan" value="{{ $permission->name }}" />
                  <x-button-save title="Simpan" />
                </form>
              </x-modal>
              <x-button-delete id="{{ $permission->id }}" title="Hapus Data" url="{{ route('backoffice.permission.destroy', $permission->id) }}" />
            </td>
          </tr>
          @endforeach
        </tbody>
  ...
  <div class="col-12 col-lg-4">
    <form action="{{ route('backoffice.permission.store') }}" method="POST">
      ...
      <x-card title="Tambah Perizinan" class="card-body">
        <x-input title="Nama Perizinan" name="name" type="text" placeholder="Masukan Nama Perizinan" value="{{ old('name') }}" />
        <x-button-save title="Simpan" />
      </x-card>
    </form>
  </div>
